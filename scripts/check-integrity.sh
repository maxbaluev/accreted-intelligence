...
if command -v node >/dev/null 2>&1; then
  ...
  if node scripts/prepare-social-launch-packet.js --decision-packet | grep -q 'Current Channel Holds'; then note "social launch channel holds: ok"; else fail=1; fi
...
if command -v node >/dev/null 2>&1; then
  ...
  actionable_followup=$(printf '%s\n' '| # | List | Area | State at refresh | PR | Last update | Note |' '|---|---|---|---|---|---|---|' '| 1 | example/list | MCP directory | open | https://github.com/example/list/pull/1 | 2026-06-17T00:00:00Z | Failing checks; inspect before any owner-approved fix or reply. |' | node scripts/prepare-directory-followup-kit.js --markdown --actionable - 2>/dev/null)
  if printf '%s\n' '| # | List | Area | State at refresh | PR | Last update | Note |' '|---|---|---|---|---|---|---|' '| 1 | example/list | MCP directory | open | https://github.com/example/list/pull/1 | 2026-06-17T00:00:00Z | Failing checks; inspect before any owner-approved fix or reply. |' | node scripts/prepare-directory-followup-kit.js --check --actionable - >/dev/null &&
    printf '%s\n' "$actionable_followup" | grep -q "Owner-review next step:" &&
    printf '%s\n' "$actionable_followup" | grep -q "Inspect the failing checks before any reply"; then
...
if [ -f "$live_workflow" ] &&
  grep -q 'expected_head' "$live_workflow" &&
  grep -q 'Verify expected checkout commit' "$live_workflow" &&
  ...
  grep -q 'expected_head=$head_sha' scripts/run-approved-growth-rollout.sh &&
