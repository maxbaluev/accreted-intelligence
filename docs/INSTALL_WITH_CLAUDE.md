...
1. **Explain before touching anything.** Claude states what acc is (a local Work Model +
...
2. **Scoped consent, then the one-liner.** With your "go", Claude's first move is handing
   ...
   - **Linux x86_64 / aarch64 / macOS** → the native `./install.sh`. It is a
     **deterministic phase-machine**: it probes your host, selects the embedder tier (the
     3-model ladder, below), auto-installs deps (Rust, `uv`, `python3`, sandbox/build
     deps, and browser system libs where supported), builds `acc`, materializes the encoder
   ...
   - **Windows** → the native `install.ps1`, the same deterministic phase-machine in
     ...
     lanes only — AWQ/triton has no Windows wheels), automatic Rust/`uv` plus `winget`
     attempts for Git and MSVC Build Tools when missing, and the same `acc doctor --json`
     handoff. The engine is Windows-native (zero `std::os::unix` outside the platform
     boundary; daemon IPC = TCP loopback + owner token); the native installer also
     provisions the Camoufox browser venv and uses the same TCP-loopback + token endpoint
     contract as the Rust client. The container below remains the fallback for
     locked-down hosts.
   ...
   profile beyond what the installer itself does. The installer names those side effects
   before running: OS packages, `winget` packages, shell PATH additions, browser libs, and
   model downloads all appear as phases. Before any real install, Claude can run `./install.sh --dry-run` (walks every
   phase, mutates nothing, reports what *would* happen and which tier it would pick) as a
   no-side-effect preview.

   > **Agent-driven path (Claude Code / Codex running the command in its own shell).** On a
   > **first** install the command can take **minutes** — the embedder model is several GB
   > and downloads on first run. An agent's shell tool usually enforces a per-command
   > **timeout** and **captures** stdout, so the install must not block on that download or
   > corrupt captured output. It doesn't: the installer **auto-detects a non-interactive
   > shell** (stdout is not a TTY, or `ACC_NONINTERACTIVE=1`) and **does not block on the
   > embedder warm-up** — it finishes the critical path (binary + wiring), prints a clear
   > `…warming in the background…` line, and **returns promptly**; the model keeps
   > downloading in the background and you check it later with `acc doctor`. Output in this
   > mode is **plain ASCII** (no ANSI color codes or Unicode glyphs) so a captured stream
   > stays clean. If you would rather watch the full live warm-up, run the command yourself
   > in a **real terminal** (a TTY keeps the interactive wait), or run it **backgrounded**
   > / with an **extended timeout** from the agent. The **CLI lane works immediately** with
   > no restart — `acc retrieve` / `acc ingest` / `acc act` are live from the shell as soon
   > as the embedder reports `ok` in `acc doctor`; only the Claude Code MCP wiring needs a
   > restart (or `/mcp`).
...
| `prereq_rust` / `prereq_uv` / `prereq_python` / `prereq_git` | ensures the toolchain (cargo, uv, python3, and Git where that OS lane needs it) — idempotent |
| `sysdeps_sandbox` / `sysdeps_msvc` | bubblewrap (Linux sandbox), a C linker, and Windows MSVC Build Tools when source fallback needs them |
...
| `browser` | optional Camoufox host-side browser env (`ACC_NO_BROWSER=1` to skip). POSIX and native Windows provision it; Windows uses the TCP-loopback + token endpoint contract. |
| `seed` | waits for the embedder, then converges the browser daemon + seeds `runtime:browser` when the browser lane is available |
| `mcp_wiring` | dev-clone only: writes/merges this repo's own project-local `.mcp.json` (idempotent). On a public install the global wiring below handles Claude Code — no project step. |
| `hosts_sync` | `acc hosts-sync` — wires **all four agents (Claude Code, OpenCode, Codex, Cursor) GLOBALLY** onto the ONE global `acc.db`: for Claude Code the user-scope `~/.claude.json` MCP server + the `~/.claude/settings.json` hooks (add-only, backed up, drift reported). `acc hosts-sync --project .` is the optional per-project isolation override. |
...
3. **Verify against `acc doctor --json` — the verification contract.** This is the
   ...
   **The eight checks (always in this order), each a layer of the organism:**
   ...
   | `substrate` | the `acc.db` opens; entity count + pinned model | run `./install.sh`, or ingest one entry to create it |
   ...
   | `mcp` | acc is reachable from Claude Code — wired GLOBALLY (`~/.claude.json` `mcpServers.acc`) or via a project `.mcp.json` | run `acc hosts-sync` (wires all agents globally on one Work Model), then reload MCP |
...
5. **Optional, consent-gated grounding.** Once doctor is green, Claude *offers* to seed a
   little starter context — an entry or two about you / your project so the first
...
acc needs **no API key and no login at all** — not for the Work Model, and not for the brain.
`acc_retrieve` and the scored-token substrate are entirely local, and the brain is the
**interactive session itself**: when the Work Model can't answer, `acc_act(solve)` returns a
...
- **Local-only by default.** The substrate (`acc.db`) is a plain file on your disk,
  ...
  sent anywhere for the Work Model or retrieval.
...
### Letting the agent act on your behalf

By default the agent **drafts and prepares** things and browses **read-only** — all with zero
setup, and nothing ever leaves your machine. When you want it to actually **send, post, pay, or
delete** something, it shows you the **exact action** and you confirm it right here in the chat
before it runs — no keys, no passphrase, no setup. The agent can only do irreversible things in a
live session with you: in an unattended / automated run it stops and waits for your ok instead.
Your word in the conversation *is* the authority.
...
- **`mcp` warn — acc not reachable from Claude Code.** Run `acc hosts-sync` (wires all your
  agents — Claude Code included — globally on the one global Work Model: `~/.claude.json` MCP
  server + `~/.claude/settings.json` hooks), then reload MCP in Claude Code so the two verbs
  appear. Use `acc hosts-sync --project .` only to isolate a project on its own db.
...
| Windows | native `install.ps1` | same phase-machine in PowerShell; Camoufox browser automation uses the native TCP-loopback + token endpoint, with the **container** ([here](INSTALL_CONTAINER.md)) still available as the no-native-deps fallback |
